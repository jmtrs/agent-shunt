...
- **`--semantic`** — a persistent, disk-cached embedding index over the whole repository recalls code the term search missed. The strongest lexical result stays authoritative; at most one new semantic region is admitted, and only when the leading semantic file is clearly separated from the next distinct file. That region then competes with the lexical tail under the same token budget. Embeds either through a provider that serves `/embeddings` (`embeddingProvider: "http"`, the default) **or fully on-device with no key or network** (`embeddingProvider: "local"`, a keyless ONNX model — build with `--features local-embed`). Vectors cache under `~/.cache/agent-shunt/index`, keyed by file content and model, so only changed files are re-embedded.
